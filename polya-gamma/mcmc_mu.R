...
mcmc_mu <- function (y, locs, K, message = 100,
  ...
  for (k in 2:K) {
    ...
    for (j in 1:(J-1)) {
      ...
      mu_tilde <- c(Sigma_tilde %*% (Sigma_inv %*% rep(mu[k,j], N) + kappa[, j]))
    ...
    mu_mh1 = sum(sapply(1:(J-1), function(j){
      dmvn(eta[k,,j], rep(mu_prop[j], N), Sigma, log=TRUE)})) + 
      sum(dnorm(mu_prop, mu0, mu0_Sigma, log=TRUE))
    mu_mh2 = sum(sapply(1:(J-1), function(j){
      dmvn(eta[k,,j], rep(mu[k,j], N), Sigma, log=TRUE)})) + 
